# multitimeframe adx approach ,,but take analysis only after closing of higher timeframe candle 
# threshold value >25 or 30 -strong trend ,

# price goes up ,adx goes down ,reversal sign ,weak trend ,

# price goes down ,adx goes down,reversal sign,weak trend

# price goes up ,adx goes up ,no reversal sign,strong trend
# price goes down ,adx goes u, no reversal sign,strong trend
...
# Calculate ADX and improve signal generation
def calculate_daily_indicators(df):
    try:
        logging.info("Calculating EMA5, EMA900, ATR, OBV, ADX, and volume-based Benford's Law")
...
def generate_signals(df):
    try:
        logging.info("Generating buy/sell signals with EMA crossover, OBV analysis, Benford's Law, and ADX trend analysis")

        # Initialize signal column
        df['signal'] = 0
        adx_threshold = 25  # ADX threshold for a strong trend

        # Calculate rolling averages for the 10 most recent 30-min candles
        df['current_avg_10'] = df['Close'].rolling(window=10).mean()  # Average of current 10 candles
        df['prev_avg_10'] = df['current_avg_10'].shift(10)  # Average of previous 10 candles (shifted)

        for i in range(20, len(df)):  # Start from 20 to ensure we have enough data for rolling average calculations
            ...
            obv_trend_up = df['OBV'].iloc[i] > df['OBV'].iloc[i - 1]  # OBV trend up
            benford_ok = df['Benford_Score'].iloc[i] > 0.1  # Benford's Law check
            adx = df['ADX'].iloc[i]  # ADX value

            # Current and previous 10-period rolling averages
            current_avg_10 = df['current_avg_10'].iloc[i]
            prev_avg_10 = df['prev_avg_10'].iloc[i]

            # Generate signals based on ADX, trend, and rolling averages
            if crossover_up and obv_trend_up and benford_ok:
                if adx > adx_threshold:
                    if current_avg_10 > prev_avg_10:
                        logging.info(f"Strong uptrend signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = 1  # Strong buy signal
                    elif current_avg_10 < prev_avg_10:
                        logging.info(f"Reversal signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = -1  # Sell signal for reversal

                elif adx < adx_threshold:
                    if current_avg_10 > prev_avg_10:
                        logging.info(f"Weak uptrend signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = -1  # Sell signal for weak trend
                    elif current_avg_10 < prev_avg_10:
                        logging.info(f"Reversal signal at {df.index[i]} with weak trend at ADX {adx}")
                        df.at[df.index[i], 'signal'] = 1  # Buy signal for reversal

            elif crossover_down and not obv_trend_up and benford_ok:
                if adx > adx_threshold:
                    if current_avg_10 > prev_avg_10:
                        logging.info(f"Strong downtrend signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = -1  # Strong sell signal
                    elif current_avg_10 < prev_avg_10:
                        logging.info(f"Reversal buy signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = 1  # Buy signal for reversal

                elif adx < adx_threshold:
                    if current_avg_10 > prev_avg_10:
                        logging.info(f"Weak downtrend signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = 1  # Buy signal for weak trend reversal
                    elif current_avg_10 < prev_avg_10:
                        logging.info(f"Weak downtrend signal at {df.index[i]} with ADX {adx}")
                        df.at[df.index[i], 'signal'] = -1  # Weak sell signal

        logging.info("Signal generation with ADX and rolling averages complete")
        return df
